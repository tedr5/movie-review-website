...
<html>
  ...
  <body>
    ...
    <div class="box_suppression">
      <form class="f_suppression" method="post" action="..\controleurs\contr_supprimer_film.php">
        <h1>Supprimer un film</h1>
        <?php
          $requete = mysqli_query($co, "SELECT * FROM Film") or die ("Execution de la requete impossible".mysqli_error($co));
          while ($data = mysqli_fetch_assoc($requete)) {
            echo '<p class="input_film"><input name="film" type="checkbox" value="'.$data['nomFilm'].'">&nbsp'.$data['nomFilm'].'</p><br>';
          }
        ?>
        <p>
